    ...
    while true; do
        ...
        for user in $vmess_users; do
            ...
            if [[ "$current_date" > "$expiration_date" ]]; then
                ...
                jq --arg uuid "$uuid" 'del(.inbounds[3].settings.clients[] | select(.id == $uuid))' /usr/local/etc/xray/config.json > temp.json && mv temp.json /usr/local/etc/xray/config.json
        ...
        for user in $vless_users; do
            ...
            if [[ "$current_date" > "$expiration_date" ]]; then
                ...
                jq --arg uuid "$uuid" 'del(.inbounds[0].settings.clients[] | select(.id == $uuid))' /usr/local/etc/xray/config.json > temp.json && mv temp.json /usr/local/etc/xray/config.json
        ...
        for user in $trojan_users; do
            ...
            if [[ "$current_date" > "$expiration_date" ]]; then
                ...
                jq --arg password "$password" 'del(.inbounds[2].settings.clients[] | select(.password == $password))' /usr/local/etc/xray/config.json > temp.json && mv temp.json /usr/local/etc/xray/config.json
